...
const CustomerView = () => {
  ...
  const reloadFeedbacks = async (locationIds: string[]) => {
    ...
    const feedbackMap: Record<string, FeedbackItem[]> = {};
    // Pre-seed every requested location with an empty list so that deleting the
    // last comment actually clears it — otherwise the merge below would keep the
    // stale previous value (comment stayed visible until manual refresh).
    for (const id of locationIds) feedbackMap[id] = [];
    if (!response.error) {
  ...
  const triggerNotification = async (changeType: "approval" | "comment") => {
    ...
    // The backend expects `event` with values comment/completion. Map the
    // frontend's change type accordingly (previously sent `changeType`, which
    // the function ignored → no mail was ever sent).
    const event = changeType === "approval" ? "completion" : "comment";
    try {
